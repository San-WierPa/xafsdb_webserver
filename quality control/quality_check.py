...
from larch import Group, fitting, xafs
...
class check_quality(object):
    ...
    def plot_k(self, show=False, save_path=None):
        ...
        self.ax_k.set_ylim(-np.max(self.data.chi) - 0.01, np.max(self.data.chi) + 0.01)
        if show:
            self.fig_k.show()
        if save_path:
...
if test_cq:
    ...
    for file in files:
        ...
        fig_data = cq.plot_background(show=False, save_path=None)
        fig_data_base64 = cq.encode_base64_figure(fig_data)
        fig_k = cq.plot_k(show=False, save_path=None)
        fig_k_base64 = cq.encode_base64_figure(fig_k)
        fig_R = cq.plot_R(show=False, save_path=None)
