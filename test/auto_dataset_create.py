...
class AutoDatasetCreation(object):
    """
    This class automatically creates a dataset, does the quality control
    and serves all to the webserver
    """
    def __init__(self):
        print('Dataset creation initialized')
        self.configuration = scicat_py.Configuration(
            host="http://35.233.84.253",
        )
        self.qc_path = "/home/sepa/Desktop/xafs-db/xafsdb/webserver/quality control/"
    ...
    def auth_login(self) -> str:
        ...
        with scicat_py.ApiClient(configuration=self.configuration) as api_client:
            ...
            credentials_dto = scicat_py.CredentialsDto(username=env("USERNAME_AUTH"), password=env("PASSWORD_AUTH"))
    ...
    def create_testdata(self):
        """
        Create dataset with quality control data
        """
        titlename = input("Enter name of dataset title: ")
        self.dummy_data2={"owner_group":"University of Wuppertal",
            "access_groups":"NO_THUMBNAIL",
            "creation_location":"wuppertal",
            "principal_investigator":"someone",
            "dataset_name": titlename + " " + datetime.now().strftime("%Y-%m-%d-%H_%M_%S"),
            "type":"raw",
            "is_published":False,
            ...
            "scientific_metadata":{
                ...
                "RAW": {"edge_step": {"value": 0.,
                        ...
                        "energy_resolution": {"value": 0.,
                                              "unit": "eV",
                                              "documentation": "energy resolution of the measured spectrum in eV"}},
                "PROCESSED": {"amplitude_reduction_factor": {"Z_ranges": {"tentoforty": {"value": 0.,},
                                                                          "fortytoeighty": {"value": 0.,}                    },
                                                             "unit": "1",
        ...
        with scicat_py.ApiClient(self.configuration) as api_client:
            ...
            api_response = api_instance.datasets_controller_create(create_dataset_dto,async_req=False,_preload_content=False)
    ...
    def post_sthree(self):
        ...
        files = {'file': open(self.qc_path + "example data/LABORATORY/" + filename,'rb')}
        values = {"dataset_id": self.datasetId}
        self.responds = requests.post('http://35.233.84.253/file/file/', files=files, data=values)
    ...
    def qc_and_update(self):
        ...
        with scicat_py.ApiClient(self.configuration) as api_client:
            ...
            cq_json = self.qc_path + 'Criteria.json' ### loading the json criteria file for reference
            cq = check_quality(quality_criteria_json = cq_json) ### initialize quality check object
            qc_list = [] ### this list will contain the quality criteria
            meas_data = np.loadtxt(self.short_url, skiprows=1) ### here the measurement data has to be forwarded, where will the data be stored in SciCat?
            ...
            fig_data = cq.plot_background() ### plot the data and return the figure object
            fig_k = cq.plot_k() ### plot data in k and return the figure object
            fig_R = cq.plot_R() ### plot data in R and return the figure object
            self.dummy_data2['scientific_metadata']['Figures']['data'] = cq.encode_base64_figure(fig_data)
            ...
            self.dummy_data2['scientific_metadata']['RAW']['edge_step']['value'] = qc_list[0][1] ### add to dummy data
            qc_list.append(cq.check_energy_resolution()) ### check for the energy resolution
            self.dummy_data2['scientific_metadata']['RAW']['energy_resolution']['value'] = qc_list[1][1] ### add to dummy data
            qc_list.append(cq.check_k())
            self.dummy_data2['scientific_metadata']['RAW']['k_max']['value'] = qc_list[2][1] ### add to dummy data
            #print(dummy_data2)
            if all(qc_list):
                print("DO SERVER COMMUNICATION -> CREATE DATASET")
                update_dataset_dto = scicat_py.UpdateDatasetDto(source_folder = self.short_url) # UpdateDatasetDto |
                api_response = api_instance.datasets_controller_find_by_id_replace_or_create(self.datasetId, update_dataset_dto, async_req=False,_preload_content=False,)
                response = json.loads(api_response.data)
                #pprint(response)
                self.data_fig=self.dummy_data2['scientific_metadata']['Figures']['data']
    ...
    def upload_data(self):
        ...
        """
        QC_attach_data={"thumbnail": self.data_fig,
        ...
        with scicat_py.ApiClient(self.configuration) as api_client:
            ...
            api_response = api_instance.datasets_controller_create_attachment(self.datasetId, create_attachment_dto, async_req=False,_preload_content=False)
            response = json.loads(api_response.data)
            #pprint(response)
    ...
    def upload_k(self):
        ...
        """
        QC_attach_k={"thumbnail":self.k_fig,
               "caption": "some caption",
                "access_groups":"None",
                "created_by": "string",
                "updated_by": "string",
    ...
    def upload_R(self):
        ...
        """
        QC_attach_R={"thumbnail":self.R_fig,
               "caption": "some caption",
                "access_groups":"None",
                "created_by": "string",
                "updated_by": "string",
